...
orders_df = pd.read_csv('maria_orders_processed.csv')
...
days_columns = [f'open_{i}' for i in range(1, 91)] + \
               [f'high_{i}' for i in range(1, 91)] + \
               [f'low_{i}' for i in range(1, 91)] + \
               [f'volume_{i}' for i in range(1, 91)]
...
for ticker in unique_tickers:
    if isinstance(ticker, str):  # Check if ticker is not NaN
        ...
        for index, order in ticker_orders.iterrows():
            ...
            # Get the last 90 elements leading up to the order execution date
            index_start = max(0, closest_index - 89)  # Ensure not to go below 0
            last_90_days_data = ticker_price_data.iloc[index_start:closest_index + 1]

            # Assign price and volume values to the respective columns
            for i, row in last_90_days_data.iterrows():
                matching_day = closest_index - i + 1
...
orders_df.to_csv('updated_maria_orders_processed.csv', index=False)
